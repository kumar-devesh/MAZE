...
class PretrainedVideoSwinTransformer(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.backbone = SwinTransformer3D()
        self.head = I3DHead(400, 1024)
    
        checkpoint = torch.load(args.video_swin_ckp)
        # print(checkpoint)
        ...
        for k, v in checkpoint['state_dict'].items():
            if 'backbone' in k:
                backbone_name = k[9:]
                backbone_state_dict[backbone_name] = v 
            elif 'cls_head' in k:
                # print('head_found')
                head_name = k[9:]
                head_state_dict[head_name] = v
