...
class SpatialDiscriminator(nn.Module):
    ...
    def forward(self, x, class_id):
        ...
        print("after reshaping", out.size())
...
if __name__ == '__main__':
    print("Training Disrciminator for 10 iterations")
    batch_size = 2
    n_frames = 4
    n_class = 400
    n_chn = 4
    ...
    for i in range(1):
        data = torch.randn((batch_size, n_frames, 3, 224, 224)).cuda()

        label = torch.randint(0, n_class, (batch_size,)).cuda() #generated labels
        ...
        print("discriminator model output: ", out.size())
        loss = torch.mean(out)
        print(loss.item())
