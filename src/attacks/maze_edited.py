...
def maze_edited(args, T, S, train_loader, test_loader, tar_acc):
    ...
    for i in pbar:
        ...
        label_set = torch.empty((32), requires_grad = True)
        l_counter = 0
        ...
        for g in range(args.iter_gen):
            ...
            if args.white_box:
                ...
                T_out = T(x)
                T_out1 = torch.argmax(T_out[0,:])
                T_out2 = torch.argmax(T_out[1,:])
                label_set[l_counter] = T_out1 
                label_set[l_counter+1] = T_out2
                l_counter += 2
                if l_counter == 32:
                    l_counter = 0
            ...
            loss_entropy = entropy(label_set)
            loss_entropy = 1/(1 + loss_entropy)

            lossG = lossG_dis + (args.alpha_gan * lossG_gan) + loss_entropy
            optG.step()
